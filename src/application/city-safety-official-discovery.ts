...
function yellowRuntime(error: unknown): YellowReason | undefined {
  if (error === null || typeof error !== "object" || types.isProxy(error) || Object.getPrototypeOf(error) !== OfficialSourceDiscoveryError.prototype) return undefined;
  const code = Object.getOwnPropertyDescriptor(error, "code"); const runtime = Object.getOwnPropertyDescriptor(error, "runtimeCode");
  if (code?.value !== "official_source_discovery_runtime_failed" || runtime === undefined || !("value" in runtime) || typeof runtime.value !== "string" || !YELLOW.has(runtime.value)) return undefined;
...
export function createCitySafetyOfficialDiscoveryAdapter(port: OfficialSourceDiscoveryPort): CitySafetyOfficialDiscoveryPort {
  return Object.freeze({ async discover(input: Input): Promise<CitySafetyOfficialDiscoveryResult> {
    ...
    for (const round of [1, 2] as const) try {
      const result = reviewedResult(await port.discover(reconstructOfficialSourceDiscoveryRequest({ schemaVersion: "official-source-discovery-request@1", entity: { entityId: city.cityId, kind: "city", countryCode: "SI", displayName: city.officialCityNames[0]! }, fact: { factKey: "si-city-safety", definitionId: plan.definitionId, description: "Municipal police offences per 100000 residents" }, failedSource: { url: canonicalHttpsUrl(input.failedUrl), reason: input.reason }, authorityRoots, localeHints: ["sl", "en"], round, signal: input.signal })) as OfficialSourceDiscoveryResult);
      if (result.urls.length > 0) return Object.freeze({ kind: "candidates", urls: result.urls, metadata: result.metadata });
