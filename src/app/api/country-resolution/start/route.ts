...
type StartCountryResolution = Pick<CountryResolutionApplication, "startCountryResolution">;

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

const requestSchema = z.object({
  automaticShortlistSnapshotId: z.string().min(1),
}).strict();
...
function applicationError(error: unknown): Response {
  ...
  return problem(500, "internal_error", "Не удалось открыть разрешение стран");
}
...
async function application(): Promise<StartCountryResolution> {
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    const readModel = await (await application()).startCountryResolution(input);
