...
An incomplete catalog artifact, manifest, trust policy, definition or validator keeps the artifact package unavailable. A completed source contract may resolve individual cities to evidence-backed unknown without blocking Start or selectability. Safety may advance from `candidate_available_with_partial_official_coverage` to `available_with_partial_official_coverage` even when some cities resolve to unknown. Missing catalog artifacts or incomplete per-member plan policy block publication; a completed plan returning unknown does not. Pure Catalog/Criteria/Ranker and the separate safety component plan may proceed against synthetic/domain fixtures; do not claim integrated VS-4A success, add a placeholder URL, build a universal crawler or use fixtures as production facts.
