...
- [ ] **Step 6: Write failing full replay test.** It must verify evidence, rerun assessment/formulas through the exact assessor selected by the sealed assessment-rules version, call Branch `replayCommit`, append nothing and label output historical. A canonical rule change requires a new version; supported historical versions remain exact, unknown versions fail before Decision projection.
- [ ] **Step 7: Run the replay test; expect FAIL, then implement application coordination.** On branch creation append a new HMAC-sealed `RunRevision(stage:"branch", parentRevisionId=assessmentRevision)` binding formula/output; never update the assessment revision. Keep only the explicit small version dispatcher required by stored VS-1 revisions—no generic migration/rules framework.
...
**Files:** Create `public/world-map.svg`, `src/application/{present-run,present-journey,verified-evidence}.ts`, `src/experience/view-model.ts`, `src/experience/components/{Vs1Start,Vs1Journey,ProfileCard,ResearchMap,LifeBranch,EvidencePassport,LifeGitDiff}.tsx`, `src/infrastructure/narrative.ts`, `src/app/{layout,page}.tsx`, `src/app/globals.css`, `tests/integration/{experience,journey-actions,present-journey}.test.tsx`; modify `src/application/contracts.ts`, `src/infrastructure/composition-root.ts`, `src/app/actions.ts`, `src/decision/{profile,assessment}.ts`, `src/research/contracts.ts`.

**Interfaces:** Application owns `NarrativePort.select({claimIds,typedValues})` and `presentRun(runId): Promise<RunDetails>`; the adapter may return only phrase IDs plus supporting claim IDs, while Application renders fixed copy. Experience consumes that serializable `RunDetails` only and never reads ports/SQLite/raw artifacts. The exact five public actions are `startConfirmedLife(draft,initialHousing)`, `retryConfirmedLifeRun(previousRunId)`, `saveInitialHousingBranch(runId)`, `rewindHousingBranch(commitId): BranchCursor`, and `forkHousingBranch(cursor,housingAll)`.

- [ ] **Step 1: Write failing confirmation/map tests.** The `/` pre-run form accepts only the current strict synthetic `ProfileDraft` plus initial housing, explicitly records twelve-month income continuation and acceptance of the lawful-stay prerequisite, and records the staged-after-sponsor condition only for a spouse route. These booleans belong to the immutable profile/hash; missing conditions keep the result yellow. The form requires final snapshot confirmation, invalidates that confirmation after every profile/housing edit, and calls the real `startConfirmedLife` Promise. While that Promise is pending it shows full-screen gray, airplane Russia→Tirana and exact one-candidate coverage; terminal state renders the returned marker. The resulting profile card is read-only and shows exact housing, monthly RUB income, income basis, resources and companion scenario without calling a scenario acceptance official proof. All markers have icon plus textual state label; green collapses map/no popover. Yellow/red native controls reveal blocker/reason through keyboard activation; a link appears only when the reason is actually source-backed, while a declined user scenario condition carries no invented official lineage. Yellow retry creates a new run/snapshot while the old run stays unchanged and successful start/retry replace `?run=` with the safely encoded server run ID.
...
- [ ] **Step 4: Add failing branch/Passport UI cases.** Render three native budget meters from exact server strings, six evidence classes, one human-readable card per official source, verified links, raw claim values/IDs/anchors only inside secondary technical details, calculation lineage and human-readable source blockers. Drive real actions `save bound C0(70000) -> fork C1(90000) -> rewind C0` and assert both the returned causal diff and full visual restoration of the saved C0 budget/diff state without mutating `C0`; reject direct C2 creation unless the UI has rewound to C0.
- [ ] **Step 5: Implement `Vs1Start`, `ProfileCard`, `LifeBranch`, `EvidencePassport`, `LifeGitDiff`, retry wiring and all five thin public actions.** Actions validate drafts/IDs/decimal input and delegate to Task 4/5 use cases; `saveInitialHousingBranch` reads the HMAC-bound run-revision value rather than client housing; no client-side branch mutation or calculation. Application owns branch loading, binding validation and C0/C1 presentation; composition root only wires ports. Guard branch actions synchronously so a pending C0 append cannot be submitted twice.
- [ ] **Step 6: Add failing narrative tests.** Missing key, timeout/refusal, invalid schema, unknown phrase/claim ID, unsupported prose, or `unknowns_explicit` without an actual unknown item returns deterministic copy; outbound payload has no PII/free text; evidence/assessment remain byte-equal.
- [ ] **Step 7: Implement application `presentRun`, semantic evidence projection, composition wiring and the server-only `responses.parse` adapter with Zod Structured Output, `store:false` and no tools.** Application accepts only its small phrase-ID allowlist, validates supporting claim-ID subsets and allows unknown copy only when unknown evidence exists; deterministic fallback never claims unknowns that are absent. Only semantically validated official claims may enter Passport/narrative/assessment; sealed-but-invalid source content becomes a typed unknown, and every sealed Assessment reason uses the claim ID of its own official source.
