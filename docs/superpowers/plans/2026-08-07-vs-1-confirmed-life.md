...
- [ ] **Step 1: Write failing confirmation/map tests.** The `/` pre-run form accepts only the current strict synthetic `ProfileDraft` plus initial housing, requires explicit confirmation, and calls the real `startConfirmedLife` Promise. While that Promise is pending it shows full-screen gray, airplane Russia→Tirana and exact one-candidate coverage; terminal state renders the returned marker. The resulting profile card is read-only and shows exact housing, monthly RUB income, income basis, resources and companion scenario without fabricating lawful-stay state. All markers have icon plus textual state label; green collapses map/no popover. Yellow/red native controls reveal blocker/reason plus official link through keyboard activation; yellow retry creates a new run/snapshot while the old run stays unchanged and successful start/retry replace `?run=` with the safely encoded server run ID.
