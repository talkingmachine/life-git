...
- [ ] **Step 1: Write failing confirmation/map tests.** The `/` pre-run form accepts only the current strict synthetic `ProfileDraft` plus initial housing, explicitly records twelve-month income continuation and acceptance of the lawful-stay prerequisite, and records the staged-after-sponsor condition only for a spouse route. These booleans belong to the immutable profile/hash; missing conditions keep the result yellow. The form requires final snapshot confirmation and calls the real `startConfirmedLife` Promise. While that Promise is pending it shows full-screen gray, airplane Russia→Tirana and exact one-candidate coverage; terminal state renders the returned marker. The resulting profile card is read-only and shows exact housing, monthly RUB income, income basis, resources and companion scenario without calling a scenario acceptance official proof. All markers have icon plus textual state label; green collapses map/no popover. Yellow/red native controls reveal blocker/reason plus official link through keyboard activation; yellow retry creates a new run/snapshot while the old run stays unchanged and successful start/retry replace `?run=` with the safely encoded server run ID.
...
- [ ] **Step 4: Add failing branch/Passport UI cases.** Render three native budget meters from exact server strings, six evidence classes, one human-readable card per official source, verified links, raw claim values/IDs/anchors only inside secondary technical details, calculation lineage and human-readable source blockers. Drive real actions `save bound C0(70000) -> rewind -> fork C1(90000)` and assert the returned causal diff without mutating `C0`; reject direct C2 creation unless the UI has rewound to C0.
- [ ] **Step 5: Implement `Vs1Start`, `ProfileCard`, `LifeBranch`, `EvidencePassport`, `LifeGitDiff`, retry wiring and all five thin public actions.** Actions validate drafts/IDs/decimal input and delegate to Task 4/5 use cases; `saveInitialHousingBranch` reads the HMAC-bound run-revision value rather than client housing; no client-side branch mutation or calculation. Application owns branch loading, binding validation and C0/C1 presentation; composition root only wires ports.
