...
**Source-shape amendment:** [`Slovenia Official Source Shape Repair`](./2026-08-11-slovenia-official-source-shape-repair.md) supersedes Task 2's fixture grammar. Task 6 resumes only after that repair passes its completion gate.
...
- The Slovenia plan has three country source bundles plus the already installed CBR EUR/RUB source: route (`GOV.SI + ZTuj-2`), income (`PISRS publication + complete SiStat listing/series`), and companion employment (`ESS + ZZSDT`). At most six discovered candidates and eleven HTTP captures are allowed.
...
- Current-run limits are 60 seconds, concurrency three, one retry token per planned source bundle only for timeout/429/5xx, 30 MiB per artifact, six discovery candidates and eleven request steps including retries.
...
- [ ] **Step 5: Audit privacy and cost.** Assert the exact discovery/model payload contains only country/roots/claim kinds; nonterminal events contain no profile; no operational record includes free text/PII. Record model calls `<=1`, candidates `<=6`, captures `<=11`, concurrency `<=3` and total current-run budget `<=60s`.
