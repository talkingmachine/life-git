...
LLM предлагает варианты и объясняет их. Детерминированные ограничения, расчёты и evidence gates
могут запретить LLM выдать неподтверждённое утверждение как факт. Финальный выбор всегда делает
пользователь.
...
- **Official-only:** внешний факт требует применимого официального источника; память LLM, поисковый
  сниппет и пересказ третьей стороны evidence не являются.
...
- микросервисы, универсальный workflow engine и несколько LLM-провайдеров;
